...
import { normalizePlan, planLabel } from './plan.js';
...
export function signActivationToken(license, deviceId) {
  const payload = {
  ...
  const options = { issuer: ISSUER, subject: String(license.id) };
  ...
  if (license.expires_at) {
    ...
    options.expiresIn = process.env.JWT_EXPIRES_IN || '365d';
  }
