...
import { signActivationToken, verifyActivationToken } from '../_utils/jwt.js';
import {
  STATUS,
  computeExpiry,
  isExpired,
...
export async function findByKey(licenseKey) {
...
export async function verifyLicense({ licenseKey, deviceId, appVersion, androidVersion }) {
...
export async function checkLicense({ token, deviceId }) {
...
function randomKey(prefix, length) {
...
export async function generateLicenses({ plan, quantity, prefix, length }) {
  const supabase = getSupabase();
...
export async function resetDevice(licenseKey) {
...
export async function renewLicense(licenseKey, { days, plan }) {
  const license = await findByKey(licenseKey);
  if (!license) throw ApiError.notFound('Invalid license.', 'LICENSE_NOT_FOUND');

  const nextPlan = plan ? normalizePlan(plan) : normalizePlan(license.plan);
  const patch = { plan: nextPlan, duration: planDurationDays(nextPlan), status: STATUS.ACTIVATED };
...
export async function licenseDetails(licenseKey) {
  const license = await findByKey(licenseKey);
  if (!license) throw ApiError.notFound('Invalid license.', 'LICENSE_NOT_FOUND');
  const synced = await syncExpiry(license);
  return {
