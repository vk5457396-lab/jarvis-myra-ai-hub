...
import { sendToTokens } from '../_utils/fcm.js';
import { normalizePlan } from '../_utils/plan.js';

/** Devices whose bound license is currently active (not expired / disabled). */
async function activeLicenseDeviceIds(planFilter) {
  ...
  return (data || [])
    ...
    .filter((l) => (planFilter ? normalizePlan(l.plan) === planFilter : true))
    .map((l) => l.device_id);
}

/** Resolves the audience into a list of { device_id, fcm_token } targets. */
export async function resolveTargets({ target, targetValue }) {
  const supabase = getSupabase();
...
export async function dispatchNotification(payload) {
  ...
  try {
    ...
    throw error instanceof ApiError
      ? error
      : ApiError.internal('Failed to deliver notification.', 'FCM_SEND_FAILED');
  }
  ...
  const status =
    outcome.successCount === 0 ? 'failed' : outcome.failureCount ? 'partial' : 'sent';
